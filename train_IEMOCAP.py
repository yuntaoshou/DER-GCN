...
def get_train_valid_sampler(trainset, valid=0.1):
    ...
    return SubsetRandomSampler(idx[split:]), SubsetRandomSampler(idx[:split])  # train_dataset, val_dataset
...
def train_or_eval_model(model, loss_function, dataloader, epoch, optimizer=None, train=False):
    ...
    for data in dataloader:
        ...
        print(umask.size())
        _, _, _, _, _, _, _, sentence = data[:]
        # print(next(iter(sentence)))
        max_sequence_len.append(textf.size(0))
        
        # log_prob, alpha, alpha_f, alpha_b = model(torch.cat((textf, acouf, visuf), dim=-1), qmask, umask)
        log_prob, alpha, alpha_f, alpha_b, _ = model(torch.cat((textf, acouf), dim=-1), qmask, umask) # seq_len, batch, n_classes
...
def train_or_eval_graph_model(model, loss_function, dataloader, epoch, cuda, optimizer=None, train=False):
    ...
    for data in dataloader:
        ...
        textf, visuf, acouf, qmask, umask, label = [d.cuda() for d in data[:-2]] if cuda else data[:-1]
        _, _, _, _, _, _, _, sentence = data[:]

        lengths = [(umask[j] == 1).nonzero().tolist()[-1][0] + 1 for j in range(len(umask))]


        log_prob, e_i, e_n, e_t, e_l = model(textf, qmask, umask, lengths, visuf=visuf, acouf=acouf)
    ...
    return avg_loss, avg_accuracy, labels, preds, masks, avg_fscore, vids, ei, et, en, el
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dropout', type=float, default=0.8, metavar='dropout', help='dropout rate')
    ...
    print("args.graph_model:", args.graph_model)
    ...
    for e in range(n_epochs):
        ...
        if args.graph_model:
            train_loss, train_acc, _, _, _, train_fscore, _, _, _, _, _ = train_or_eval_graph_model(model, loss_function, train_loader, e, cuda, optimizer, True)
            valid_loss, valid_acc, _, _, _, valid_fscore, _, _, _, _, _ = train_or_eval_graph_model(model, loss_function, valid_loader, e, cuda)
            test_loss, test_acc, test_label, test_pred, test_mask, test_fscore, _, _, _, _, _ = train_or_eval_graph_model(model, loss_function, test_loader, e, cuda)
    ...
    #                                    round(f1_score(best_label, best_pred, sample_weight=best_mask,
    #                                                   average='weighted') * 100, 2)))
    # print(classification_report(best_label, best_pred, sample_weight=best_mask, digits=4))
    # print(confusion_matrix(best_label, best_pred, sample_weight=best_mask))
    # best_mask = np.array(best_mask).reshape(2821)
    # print('Loss {} F1-score {}'.format(best_loss,
